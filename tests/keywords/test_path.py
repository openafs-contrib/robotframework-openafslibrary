...
def test__convert_errno_parm__returns_expected_when_given_valid_arg(value, expected):
    got = _convert_errno_parm(value)
    assert got == expected


def test__convert_errno_parm__raises_exception_when_given_invalid_arg():
    with pytest.raises(AssertionError) as e:
        _convert_errno_parm("bogus")
    assert "'bogus' is not a valid errno name" in str(e)


def test_create_files__creates_a_single_file_when_given_defaults(keywords, tmp_path):
...
def test_directory_entry_should_exist__succeeds_when_dir_is_present(
    keywords,
    tmp_dir,
):
...
def test_directory_entry_should_exist__raises_exception_when_dir_is_missing(
    keywords, tmp_path
):
...
def test_should_be_file__succeeds_when_file_is_present(keywords, tmp_file):
    keywords.should_be_file(str(tmp_file))


def test_should_be_file__raises_exception_when_file_is_missing(keywords, tmp_path):
    missing = tmp_path / "missing"
    with pytest.raises(AssertionError) as e:
...
def test_should_be_symlink__succeeds_when_symlink_is_present(keywords, tmp_symlink):
    keywords.should_be_symlink(str(tmp_symlink))


def test_should_be_symlink__raises_exception_when_symlink_is_missing(
    keywords, tmp_path
):
    missing = tmp_path / "missing"
    with pytest.raises(AssertionError) as e:
...
def test_should_not_be_symlink__succeeds_when_file_is_not_a_symlink(
    keywords,
    tmp_file,
):
...
def test_should_not_be_symlink__succeeds_when_file_is_missing(
    keywords,
    tmp_path,
):
    missing = tmp_path / "missing"
...
def test_should_not_be_symlink__raise_exception_when_file_is_a_symlink(
    keywords,
    tmp_symlink,
):
    with pytest.raises(AssertionError) as e:
...
def test_should_be_dir__succeeds_when_dir_is_present(keywords, tmp_dir):
    keywords.should_be_dir(str(tmp_dir))


def test_should_not_be_dir__succeeds_when_dir_is_missing(keywords, tmp_path):
    missing = tmp_path / "missing"
    keywords.should_not_be_dir(str(missing))


def test_should_not_be_dir__succeeds_when_name_is_a_file(keywords, tmp_file):
    keywords.should_not_be_dir(str(tmp_file))


def test_should_not_be_dir__raises_exception_when_name_is_a_dir(keywords, tmp_dir):
    with pytest.raises(AssertionError) as e:
        keywords.should_not_be_dir(str(tmp_dir))
    assert "is a directory" in str(e)


def test_link__creates_hard_link_when_making_new_link(keywords, tmp_file):
...
def test_link__raises_exception_when_name_is_already_present(keywords, tmp_file):
...
def test_link__code_is_eexist_when_name_is_already_present(keywords, tmp_file):
    bogus = tmp_file.parent / "bogus"
    bogus.touch()
    keywords.link(str(tmp_file), str(bogus), code_should_be=errno.EEXIST)


def test_symlink__creates_symlink_in_same_dir_when_name_is_available(
    keywords, tmp_file
):
...
def test_unlink__removes_file_when_file_is_present(keywords, tmp_file):
    keywords.unlink(str(tmp_file))
    assert not tmp_file.exists()


def test_link_count_should_be__returns_2_when_file_has_1_hardlink(keywords, tmp_file):
...
def test_inode_should_be_equal__succeeds_when_file_is_linked(keywords, tmp_file):
    tmp_hardlink = tmp_file.parent / "tmp_hardlink"
    os.link(tmp_file, tmp_hardlink)
    keywords.inode_should_be_equal(str(tmp_file), str(tmp_hardlink))


def test_get_inode__returns_file_inode_number_when_file_is_present(keywords, tmp_file):
